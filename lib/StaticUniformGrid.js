...
function StaticUniformGrid(data, cellWidth, compareObjectBoundingBox, cellCheck) {
    ...
    console.log("model's min and max are " + min + " " + max);
    console.log("tight bound dimensions are " + Cartesian3.subtract(max, min, new Cartesian3()));
 ...
 * @param {Cartesian3} [octant] The octant to check. +1 +1 +1 indicates Octant I, -1 +1 +1 Octant II, and so on.
 ...
 */
StaticUniformGrid.forNeighborOctant = function(grid, position, octant, neighborFunction, parameters) {
    var min = grid._axisAlignedBoundingBox.minimum;
    var inverseCellWidth = grid._inverseCellWidth;
    ...
    var xStart = Math.max(Math.min(xMid, xMid + octant.x), 0);
    var yStart = Math.max(Math.min(yMid, yMid + octant.y), 0);
    var zStart = Math.max(Math.min(zMid, zMid + octant.z), 0);

    var xEnd = Math.min(Math.max(xMid + 1, xMid + octant.x + 1), resolution.x);
    var yEnd = Math.min(Math.max(yMid + 1, yMid + octant.y + 1), resolution.y);
    var zEnd = Math.min(Math.max(zMid + 1, zMid + octant.z + 1), resolution.z);
