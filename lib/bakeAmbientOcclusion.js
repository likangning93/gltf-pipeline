        ...
        if (meshes.hasOwnProperty(meshID)) {
            ...
            for (var primitiveID in primitives) {
                if (primitives.hasOwnProperty(primitiveID)) {
                    ...
                    var goalResolution = diffuseImage.width;
                    ...
                    for (var x = 0; x < goalResolution; x++) {
                        for (var y = 0; y < goalResolution; y++) {
                            diffuseImage.setPixelColor(jimpAO.getPixelColor(x, y), x, y);
                        }
                    }
                }
            }
...
function postProcessAO(aoBuffer, dataResolution, goalResolution, jimpAO) {
    ...
    for (var x = 0; x < dataResolution; x++) {
        for (var y = 0; y < dataResolution; y++) {
            var aoBufferIdx = dataResolution * y + x;
            value = 255.0 * (aoBuffer.samples[aoBufferIdx] / aoBuffer.count[aoBufferIdx]);
            value = Jimp.rgbaToInt(value, value, value, 255);
            jimpAO.setPixelColor(value, x, y);
        }
    }
...
function cloneAndSetup(newMateralID, newTextureID, newImageID,
    ...
    if (defined(newImageID)) {
        newImage = clone(images[oldImageID]);
        var pipelineExtras = newImage.extras._pipeline;
        pipelineExtras.jimpImage = pipelineExtras.jimpImage.clone();
        images[newImageID] = newImage;
    }
    if (defined(newTextureID)) {
        newTexture = clone(textures[oldTextureID]);
...
function ensureImageUniqueness(gltf, primitive, meshPrimitiveID, state) {
    ...
    return images[newImageID].extras._pipeline.jimpImage;
}
