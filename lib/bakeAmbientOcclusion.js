    ...
    while(uStep < uMax) {
        ...
        while(vStep < vMax) {
            ...
            var contribution = computeAmbientOcclusionAt(
                ...
                raytracerScene.grid, raytracerScene.nearCull, raytracerScene.rayDistance);
...
function computeAmbientOcclusionAt(position, normal, numberRays, sqrtNumberRays,
    ...
    for (var j = 0; j < numberRays; j++) {
        ...
        if (uniformGridRaytrace(triangles, sampleRay, nearCull, rayDistance)) {
            contribution += 1;
        }
    }
    return contribution;
...
var octantScratch = new Cartesian3();
function uniformGridRaytrace(uniformGrid, ray, nearCull, farCull) {
    ...
    octantScratch.x = direction.x > 0.0 ? 1 : -1;
    octantScratch.y = direction.y > 0.0 ? 1 : -1;
    octantScratch.z = direction.z > 0.0 ? 1 : -1;

    StaticUniformGrid.forNeighborOctant(uniformGrid, ray.origin, octantScratch, raytraceNeighborFunction, parameters);
