...
function generateRaytracerScene(scene, gltf, options) {
    ...
    var addNodeToSoup = function(node) {
        ...
        for (var meshIndex in meshes) {
            ...
            for (var primitiveIndex in mesh.primitives) {
                ...
                processPrimitive(rayTracerScene, mesh.primitives[primitiveIndex], aoBuffer, transform);
            }
        }
    };
    ...
    for (var nodeID in rootNodes) {
        if (rootNodes.hasOwnProperty(nodeID)) {
            var node = rootNodes[nodeID];
            nodeHelpers.depthFirstTraversal(node, gltf.nodes, addNodeToSoup);
        }
    }
...
function processPrimitive(raytracerScene, primitive, aobBuffer, transform) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        var position0 = new Cartesian3(positions[index0]);
        var position1 = new Cartesian3(positions[index1]);
        var position2 = new Cartesian3(positions[index2]);
        ...
        Matrix4.multiplyByPointAsVector(normalAvg, inverse, normalAvg);
