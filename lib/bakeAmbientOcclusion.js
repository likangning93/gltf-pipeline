...
function generateRaytracerScene(scene, gltf, options) {
    ...
    var rayTracerScene = {
        ...
        numberSamples : defaultValue(options.numberSamples, 64),
...
function addNodeToSoup(parameters, node) {
    ...
    for (var meshIndex in meshNames) {
        if (meshNames.hasOwnProperty(meshIndex)) {
            ...
            var mesh = parameters.meshes[meshName];
            ...
            for (var primitiveIndex in primitives) {
                if (primitives.hasOwnProperty(primitiveIndex)) {
                    ...
                    processPrimitive(parameters.rayTracerScene, primitives[primitiveIndex], aoBuffer, transform);
                }
            }
        }
    }
...
function processPrimitive(raytracerScene, primitive, aoBuffer, transform) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        var resolution = aoBuffer.resolution;
...
function generateOcclusionData(raytracerScene) {
    ...
    for (var i = 0; i < texelPoints.length; i++) {
        ...
        console.log(texelPoint.normal);

        // for each of N rays:
        for (var j = 0; j < numberSamples; j++) {
            var sampleRay = generateJitteredRay(texelPoint, sqrtNumberSamples);
            aoBuffer.count[aoBufferIndex]++;
            var nearestIntersect = naiveRaytrace(triangles, sampleRay);
...
function naiveRaytrace(triangleSoup, ray) {
    ...
    for (var triangleSoupIndex = 0; triangleSoupIndex < triangleSoup.length; triangleSoupIndex++) {
        ...
        if (defined(distance) && distance > 0.0) {
...
var numberRandomsGenerated = 0;

function generateJitteredRay(texelPoint, sqrtNumberSamples) {
    ...
    var s = (numberRandomsGenerated % sqrtNumberSamples) * cellWidth * Math.random() / sqrtNumberSamples;
    var t = (numberRandomsGenerated / sqrtNumberSamples) * cellWidth * Math.random() / sqrtNumberSamples;
    numberRandomsGenerated++;

    var u = 2.0 * Math.PI * s;
    var v = Math.sqrt(1.0 - t * t);

    // generate ray on a y-up hemisphere
