...
function bakeAmbientOcclusion(gltf, aoOptions) {
   var options = generateOptions(gltf, aoOptions);
    ...
    if (useUniformGrid) {
        console.log("constructing a uniform grid from " + raytracerScene.triangleSoup.length + " triangles.");
        var now = new Date().getTime();
        raytracerScene.grid = StaticUniformGrid.fromTriangleSoup(raytracerScene.triangleSoup, options.rayDistance);
        var end = new Date().getTime();
        console.log("grid constructed in " + (end - now) / 1000.0 + " seconds.");
        console.log("stats:");
        console.log("  resolution: " + raytracerScene.grid.resolution);
        console.log("  cloned data length: " + raytracerScene.grid.data.length);
        console.log("  AABB: " + raytracerScene.grid.axisAlignedBoundingBox);

        // compute average number of triangles per cell
        var cellCounts = raytracerScene.grid.cellCounts;
        var countsTotal = 0;
        var totalCounts = cellCounts.length;
        var minCounts = Number.POSITIVE_INFINITY;
        var maxCounts = Number.NEGATIVE_INFINITY;
        for (var i = 0; i < totalCounts; i++) {
            countsTotal += cellCounts[i];
            maxCounts = Math.max(cellCounts[i], maxCounts);
            minCounts = Math.min(cellCounts[i], minCounts);
        }
        console.log("max triangles per cell: " + maxCounts);
        console.log("min triangles per cell: " + minCounts);
        console.log("avg triangles per cell: " + countsTotal / totalCounts);
    }
    ...
    //console.log("correcting normals...");
    //NodeHelpers.forEachPrimitiveInScene(gltf, scene, correctNormals, parameters);
...
function correctNormals(primitive, meshPrimitiveID, parameters, node) {
    ...
    console.log("correcting normals for " + numTriangles + " triangles.");
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        scratchRay.origin = worldPositionScratch;
        scratchRay.direction = normal;
        // Raytrace
        var numISX = countIntersections(triangleSoup, scratchRay, 0.001);
        if (numISX % 2 === 1) {
            normals[index0] = Cartesian3.multiplyByScalar(normals[index0], -1.0, normals[index0]);
            normals[index1] = Cartesian3.multiplyByScalar(normals[index1], -1.0, normals[index1]);
            normals[index2] = Cartesian3.multiplyByScalar(normals[index2], -1.0, normals[index2]);
        }
    }
}
    ...
    while(uStep < uMax) {
        ...
        while(vStep < vMax) {
            ...
            var contribution = computeAmbientOcclusionAt(
                ...
                raytracerScene.triangleSoup, raytracerScene.grid, raytracerScene.nearCull, raytracerScene.rayDistance);
...
function raytraceAtTriangleCenters(primitive, meshPrimitiveID, parameters, node) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        var contribution = computeAmbientOcclusionAt(position, normal, numberRays, sqrtNumberRays,
            raytracerScene.triangleSoup, grid, raytracerScene.nearCull,
...
function countIntersections(triangleSoup, ray, nearCull) {
...
function computeAmbientOcclusionAt(position, normal, numberRays, sqrtNumberRays,
    triangles, grid, nearCull, rayDistance) {
    var contribution = 0.0;
    for (var j = 0; j < numberRays; j++) {
        var sampleRay = generateJitteredRay(position, normal, j, sqrtNumberRays);
        var nearestIntersect;
        if (//naiveRaytrace(triangles, sampleRay, nearCull, rayDistance)) {
            uniformGridRaytrace(grid, sampleRay, nearCull, rayDistance)) {
            contribution += 1;
        }
    }
    return contribution;
...
function raytraceNeighborFunction(positions, parameters) {
    ...
    parameters.hasIntersect = parameters.hasIntersect || defined(distance) && distance > parameters.nearCull && distance < parameters.farCull;
    return parameters.hasIntersect; // early return
    //return false;
}
...
function naiveRaytrace(triangleSoup, ray, nearCull, farCull) {
    // Check ray against every triangle in the soup. return the nearest intersection.
    ...
    for (var triangleSoupIndex = 0; triangleSoupIndex < triangleCount; triangleSoupIndex++) {
        ...
        if (minIntersect < farCull) {
            return true;
        } // early return
    }
    return minIntersect < farCull;
}
