...
function bakeAmbientOcclusion(gltf, aoOptions) {
   var options = generateOptions(gltf, aoOptions);
    ...
    if (useUniformGrid) {
        ...
        raytracerScene.triangleSoup = StaticUniformGrid.fromTriangleSoup(raytracerScene.triangleSoup, options.rayDistance);
...
function correctNormals(primitive, meshPrimitiveID, parameters, node) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        scratchRay.position = worldPositionScratch;
        scratchRay.direction = normal;
        // Raytrace
        var numISX = countIntersections(triangleSoup, scratchRay, 0.0001);
        if (numISX % 2 === 1) {
