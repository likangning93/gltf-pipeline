...
function bakeAmbientOcclusion(gltf, options) {
    ...
    bakeToDiffuse(gltf, raytracerScene);
...
function bakeToDiffuse(gltf, raytracerScene) {
    // find material with a diffuse texture parameter to clone as needed
    var materials = gltf.materials;
    var materialID;
    var material;
    var diffuseTextureMaterial;
    for (materialID in materials) {
        if (materials.hasOwnProperty(materialID)) {
            material = materials[materialID];
            if (defined(material.values) && defined(material.values.diffuse)) {
                if (typeof material.values.diffuse === "string") {
                    diffuseTextureMaterial = material;
                    break;
                }
            }
        }
    }
    if (!defined(diffuseTextureMaterial)) {
        throw new DeveloperError('In stage bakeAmbientOcclusion: could not find any materials with a diffuse texture.');
    }

    // build a hash of materials we've seen so far to ensure that all the primitives get unique materials
    var materialsBaked = {};
        ...
        if (meshes.hasOwnProperty(meshID)) {
            ...
            for (var primitiveID in primitives) {
                if (primitives.hasOwnProperty(primitiveID)) {
                    ...
                    // make sure each primitive has its own material with a unique texture
                    // clone the canonical one if needed
                    materialID = primitive.material;
                    var values;
                    var uniformDiffuseColor;
                    ...
                    // if the existing material has been seen, clone it.
                    if (defined(materialsBaked[materialID])) {
                        material = clone(material);
                        values = material.values;
                    } else if (!defined(values) || !(defined(values.diffuse) || !(typeof values.diffuse === 'string'))) {
                        // if the material isn't suitable for baking AO, clone diffuseTextureMaterial
                        material = clone(diffuseTextureMaterial);
                        values = material.values;
                        // if the material had a diffuse color, use that as the base of a texture with baked AO
                        if (defined(values) && defined(values.diffuse)) {
                            uniformDiffuseColor = values.diffuse;
                        }
                    } else {
                        materialsBaked[materialID] = true;
                    }

                    var textureImage;
                    // read out the texture (if any) from the data uri
                    if (defined(uniformDiffuseColor)) {
