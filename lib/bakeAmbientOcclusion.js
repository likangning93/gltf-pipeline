...
function bakeAmbientOcclusion(gltf, options) {
    // required: no primitives are instanced
        ...
        if (meshes.hasOwnProperty(meshID)) {
            ...
            for (var primitiveID in primitives) {
                if (primitives.hasOwnProperty(primitiveID)) {
                    ...
                    var aoBuffer = raytracerScene.aoBufferByPrimitive[primitiveID];
                    for (var y = 0; y < pngScratch.height; y++) {
                        for (var x = 0; x < pngScratch.width; x++) {
                            ...
                            var value = 255.0 * (aoBuffer.samples[aoBufferIdx] / aoBuffer.count[aoBufferIdx]);
...
function addNodeToSoup(parameters, node) {
    ...
    for (var meshIndex in meshNames) {
        if (meshNames.hasOwnProperty(meshIndex)) {
            ...
            for (var primitiveID in primitives) {
                if (primitives.hasOwnProperty(primitiveID)) {
                    processPrimitive(parameters, primitiveID, primitives[primitiveID], transform);
                }
            }
        }
    }
...
function processPrimitive(parameters, primitiveID, primitive, transform) {
    ...
    raytracerScene.aoBufferByPrimitive[primitiveID] = aoBuffer;
